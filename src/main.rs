...
use crate::math::{Color, Interval, Vec3};
...
fn main() {
    ...
    file.write(format!("P3\n{} {} {}\n", camera.image_width, camera.image_height, 255).as_bytes())
        .unwrap();
    ...
    world.add(Sphere::new(Vec3::new(0.0, -100.5, -1.0), 100.0, lamb1.clone()));
...
fn print_color(file: &mut File, c: &Color) {
    const INTENSITY: Interval = Interval::new(0.0, 0.99999);

    let mut r = INTENSITY.clamp(c.x);
    let mut g = INTENSITY.clamp(c.y);
    let mut b = INTENSITY.clamp(c.z);
